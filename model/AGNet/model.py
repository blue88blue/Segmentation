...
class AG_Net(nn.Module):
    ...
    def forward(self, x):
        ...
        x_2 = F.interpolate(x, size=(int(img_shape / 2), int(img_shape / 2)), mode='bilinear', align_corners=True)
        x_3 = F.interpolate(x, size=(int(img_shape / 4), int(img_shape / 4)), mode='bilinear', align_corners=True)
        x_4 = F.interpolate(x, size=(int(img_shape / 8), int(img_shape / 8)), mode='bilinear', align_corners=True)
        ...
        N, C, H, W= FG.size()
        FG_small = F.interpolate(FG, size=(int(H/2), int(W/2)), mode='bilinear', align_corners=True)
